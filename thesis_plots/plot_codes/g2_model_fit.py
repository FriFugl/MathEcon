...
def fit_gaussian(params: tuple[float, float, float, float, float], f_M: dict[float, float], P_M: dict[float, float]) -> float:
    """
    ...
    GaussianModelInstance = GaussianModel(a=a, b=b, sigma=sigma, eta=eta, rho=rho,
                                          instant_forward_rates=f_M)

    short_rates_calibration, x_calibration, y_calibration, varphi_calibration = GaussianModelInstance.simulate(T=30,
                                                                                                               M=120,
                                                                                                               N=1,
                                                                                                               method='euler')

    fitted_values = GaussianModelInstance.price_zcb(x_paths=x_calibration,
...
fit_result = minimize(fit_gaussian,
                      param_0,
                      method='L-BFGS-B',
                      bounds=bounds,
                      args=(f_M, P_M),
...
print(f"Parameters from the 2D Gaussian model fit: a = {a}, b = {b} sigma = {sigma} and eta = {eta},"
      f"rho = {rho}. Objective function of the fit: {fit_result.fun}")


instant_forward_rates = dict(zip(maturities, market_forward_rates))

GaussianModelInstance = GaussianModel(a=a,
                                      b=b,
                                      sigma=sigma,
                                      eta=eta,
                                      rho=rho,
                                      instant_forward_rates=instant_forward_rates)
short_rates, x, y, varphi = GaussianModelInstance.simulate(T=30,
                                                           M=120,
                                                           N=1,
                                                           method='euler')
fitted_zcb_prices = GaussianModelInstance.price_zcb(x_paths=x,
                                                    y_paths=y,
                                                    varphi=varphi,
                                                    t=0,
                                                    maturities=list(P_M.keys()))
...
plt.plot(maturities, result, label=f'Fitted ZCB prices', color=colors['dark_green'], alpha=0.7)
...
plt.savefig(r'G:\Mit drev\Matematik-økonomi\Kandidat\Speciale\GitHub\MathEcon\thesis_plots\plots\gaussian_zcb_fit.png', bbox_inches='tight')
plt.show()
...
fig, ax = plt.subplots(dpi=300, figsize=(8,6))
plt.plot(maturities, APE, label='Relative error', linestyle='none', marker='o', color=colors['dark_green'], alpha=0.7)
...
plt.savefig(r'G:\Mit drev\Matematik-økonomi\Kandidat\Speciale\GitHub\MathEcon\thesis_plots\plots\gaussian_relative_erros.png', bbox_inches='tight')
plt.show()