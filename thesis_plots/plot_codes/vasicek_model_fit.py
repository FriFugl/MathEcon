...
def fit_vasicek(
    params: tuple[float, float, float, float], P_M: dict[float, float]
) -> float:
    """
    ...
    short_rates_calibration = VasicekModelInstance.simulate(
        r_0=r_0, T=30, M=120, N=1, method="exact"
    )
    fitted_values = VasicekModelInstance.price_zcb(
        short_rates=short_rates_calibration, t=0, maturities=list(P_M.keys())
    )
...
print(
    f"Parameters from the Vasicek fit: r_0 = {fit_result.x[0]}, a = {fit_result.x[1]} and b = {fit_result.x[2]},"
...
fitted_zcb_prices = VasicekModelInstance.price_zcb(
    short_rates=short_rates, t=0, maturities=list(P_M.keys())
)
...
plt.plot(
    maturities, result, label=f"Fitted ZCB prices", color=colors["dark_blue"], alpha=0.7
)
...
plt.savefig(
    r"G:\Mit drev\Matematik-økonomi\Kandidat\Speciale\GitHub\MathEcon\thesis_plots\plots\vasicek_zcb_fit.png",
    bbox_inches="tight",
)
plt.show()
...
plt.savefig(
    r"G:\Mit drev\Matematik-økonomi\Kandidat\Speciale\GitHub\MathEcon\thesis_plots\plots\vasicek_relative_erros.png",
    bbox_inches="tight",
)
plt.show()
