...
def r_star_equation(r_star, T, t_i, a, b, sigma, X, tau):
    ...
    for time in t_i:
        ...
        A = ((B - time + T) * (a * b - 0.5 * (sigma**2))) / (a**2) - (
            (sigma**2) * (B**2)
        ) / (4 * a)
...
for T in expiry_dates:
    ...
    for time in t_i:
        ...
        ZBP = euro_option_price_vasicek(
            K=X_i, T1=T, T2=time, p_T1=p1, p_T2=p2, a=a, sigma=sigma, type="put"
        )
...
colors = [cfg.colors["dark_blue"]] + [
    cfg.colors["light_petrol"] for i in range(final_expiry)
]
# Plotting
...
plt.tight_layout()
plt.savefig(
    rf"G:\Mit drev\Matematik-økonomi\Kandidat\Speciale\GitHub\MathEcon\thesis_plots\plots\euro_vs_bermudan_swaption_prices_vasicek_{final_expiry}Y.png",
    bbox_inches="tight",
)
plt.show()
