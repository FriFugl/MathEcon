...
def y_bar_equation(y_bar, x, strike, T, swap_annuities, a, b, sigma, eta, rho):
    ...
    for i in range(len(swap_annuities)):
        ...
        A = (P_M[t_i] / P_M[T]) * np.exp(
            0.5
            * (
                V(T, t_i, a, b, sigma, eta, rho)
                - V(0, t_i, a, b, sigma, eta, rho)
...
def integrand(x, strike, omega, T, swap_annuities, P_M, a, b, sigma, eta, rho):
    ...
    rho_xy = ((rho * sigma * eta) / ((a + b) * sigma_x * sigma_y)) * (
        1 - np.exp(-(a + b) * T)
    )

    M_x_T = (
        ((sigma**2) / (a**2) + (rho * sigma * eta) / (a * b)) * (1 - np.exp(-a * T))
        - ((sigma**2) / (2 * a**2)) * (1 - np.exp(-2 * a * T))
        - ((rho * sigma * eta) / (b * (a + b))) * (1 - np.exp(-(a + b) * T))
    )

    M_y_T = (
        ((eta**2) / (b**2) + (rho * sigma * eta) / (a * b)) * (1 - np.exp(-b * T))
        - ((eta**2) / (2 * b**2)) * (1 - np.exp(-2 * b * T))
        - ((rho * sigma * eta) / (a * (a + b))) * (1 - np.exp(-(a + b) * T))
    )
    ...
    y_bar = fsolve(
        y_bar_equation,
        x0=0.05,
        args=(x, strike, T, swap_annuities, a, b, sigma, eta, rho),
    )[0]

    h_1 = ((y_bar - mu_y) / (sigma_y * np.sqrt(1 - rho_xy**2))) - (
        rho_xy * (x - mu_x)
    ...
    return (
        (np.exp(-0.5 * (((x - mu_x) / sigma_x) ** 2))) / (sigma_x * np.sqrt(2 * np.pi))
    ) * (ndtr(-omega * h_1) - _sum)
...
for T in expiry_dates:
    ...
    result, err = quad(
        ...
        -10,
        10,
        args=(strike, omega, T, swap_annuities, P_M, a, b, sigma, eta, rho),
    )
...
exercise_dates = [
    ...
    if i * (final_expiry / M) < final_expiry - alpha
]
...
short_rates_calibration, x_calibration, y_calibration, varphi_calibration = (
    GaussianModelInstance.simulate(T=final_expiry, M=M, N=10000, method="euler")
)
...
swap_rates_calibration, accrual_factors_calibration = GaussianModelInstance.swap_rate(
...
swap_rates_estimation, accrual_factors_estimation = GaussianModelInstance.swap_rate(
...
colors = [cfg.colors["dark_green"]] + [
    cfg.colors["light_green"] for i in range(final_expiry)
]
# Plotting
...
plt.tight_layout()
plt.savefig(
    rf"G:\Mit drev\Matematik-økonomi\Kandidat\Speciale\GitHub\MathEcon\thesis_plots\plots\euro_vs_bermudan_swaption_prices_g2_{final_expiry}Y.png",
    bbox_inches="tight",
)
plt.show()
