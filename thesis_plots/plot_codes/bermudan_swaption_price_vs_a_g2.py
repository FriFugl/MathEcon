...
g2_as = [g2_a + (i / 10000) for i in offset]
lsm_result = []
for a in g2_as:
    GaussianModelInstance = GaussianModel(
        a=a,
    ...
    short_rates_calibration, x_calibration, y_calibration, varphi_calibration = (
        GaussianModelInstance.simulate(T=T, M=M, N=10000, method="euler", seed=1)
    )

    short_rates_estimation, x_estimation, y_estimation, varphi_estimation = (
        GaussianModelInstance.simulate(T=T, M=M, N=10000, method="euler", seed=2)
    )
...
plt.plot(offset, lsm_result, color=colors["dark_green"], alpha=0.7)
...
plt.savefig(
    r"G:\Mit drev\Matematik-økonomi\Kandidat\Speciale\GitHub\MathEcon\thesis_plots\plots\bermudan_swaption_price_vs_a_g2.png",
    bbox_inches="tight",
)
