...
for final_expiry in expiries:
    ...
    short_rates_calibration = VasicekModelInstance.simulate(
        r_0=vasicek_r_0, T=final_expiry, M=M, N=10000, method="exact", seed=1
    )
    short_rates_estimation = VasicekModelInstance.simulate(
        r_0=vasicek_r_0, T=final_expiry, M=M, N=10000, method="exact", seed=2
    )
...
plt.tight_layout()
plt.savefig(
    r"G:\Mit drev\Matematik-økonomi\Kandidat\Speciale\GitHub\MathEcon\thesis_plots\plots\bermudan_swaption_price_vs_T.png",
    bbox_inches="tight",
)
plt.show()
