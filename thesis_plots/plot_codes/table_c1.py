...
for combo in combinations:
    ...
    for i in range(100):
        ...
        short_rate_IS = pd.DataFrame(
            r, index=stock_paths_IS.index, columns=stock_paths_IS.columns
        )
        discount_factors_IS = _short_rate_to_discount_factors(short_rates=short_rate_IS)

        short_rate_OOS = pd.DataFrame(
            r, index=stock_paths_OOS.index, columns=stock_paths_OOS.columns
        )
        discount_factors_OOS = _short_rate_to_discount_factors(
            short_rates=short_rate_OOS
        )

        LSM_model = LSM_method_v2(
            strike=strike, exercise_dates=exercise_dates, degree=3
        )
        calibration_payoffs = _calculate_option_payoffs(
            stock_paths=stock_paths_IS, strike=strike, call=False
        )
        estimation_payoffs = _calculate_option_payoffs(
            stock_paths=stock_paths_OOS, strike=strike, call=False
        )

        LSM_IS, LSM_betas = LSM_model.calibration(
            method="classic",
        ...
        delta_LSM_IS, delta_LSM_betas = LSM_model.calibration(
            method="stock_delta",
            underlying_asset_paths=stock_paths_IS.copy(),
            payoffs=calibration_payoffs.copy(),
            discount_factors=discount_factors_IS.copy(),
        ...
        LSM_OOS = LSM_model.estimation(
            underlying_asset_paths=stock_paths_OOS.copy(),
            payoffs=estimation_payoffs.copy(),
            discount_factors=discount_factors_OOS.copy(),
            betas=LSM_betas.copy(),
        )

        delta_LSM_OOS = LSM_model.estimation(
            underlying_asset_paths=stock_paths_OOS.copy(),
            payoffs=estimation_payoffs.copy(),
            discount_factors=discount_factors_OOS.copy(),
